...
### ADR-017: Corrección de `detalle` en gestiones de Privada — endpoint separado, evento auditado pero oculto del timeline de usuario
**Decisión**: `PATCH /api/v1/privada/gestiones/{id}/detalle` (nuevo, separado de `PATCH /gestiones/{id}`) permite corregir el campo `detalle` de una gestión directamente desde el panel general (`GestionesListPage.tsx`) para errores manuales de carga. A diferencia de `patch_gestion` (que emite `ACTUALIZA_DATO` por cada campo cambiado), esta corrección genera un evento de tipo `CORRECCION_DETALLE` — se persiste igual en `priv_gestiones_eventos` + `audit_log` (trazabilidad interna intacta), pero `listar_eventos` lo excluye explícitamente de la respuesta que alimenta el timeline de "Movimientos" del `GestionDetalleDrawer`. No dispara lógica de cambio de estado ni de negocio.
**Razón**: El área pidió que la corrección de errores de carga en `detalle` no "cuente" como una modificación de la gestión de cara al usuario — round-trip explícito con el usuario del sistema (2026-09-04) descartó la opción de no auditar nada en absoluto, precisamente porque este es un sistema de gestión de demandas de un ministerio y perder toda traza de una edición sería un riesgo de integridad/accountability mayor que el beneficio de UX. Se optó por la vía intermedia: auditado igual, invisible sólo en la vista pensada para el historial de negocio.
**Consecuencia**: `tipo_evento` en `priv_gestiones_eventos` no tiene `CHECK` (es `VARCHAR(60)` libre), así que agregar `CORRECCION_DETALLE` no requiere migración. El listado (`GET /gestiones`) ahora incluye `updated_at` en cada item (antes sólo estaba en el detalle) para poder pasar el lock optimista (`updated_at` + 409, mismo patrón que `cambiar_estado`/`patch_gestion`) desde una edición inline en la tabla, sin tener que pedir el detalle completo primero. Desplegado 2026-09-04: `svc-privada` revisión `svc-privada-00005-cfh` + gateway `ministerio-config-v20260904` + frontend en producción.
**Consecuencia (integridad histórica)**: la fila `CORRECCION_DETALLE` sigue siendo consultable a nivel de base de datos (o de una futura vista de auditoría admin) — nunca se pierde el valor anterior, sólo se filtra de la UI de negocio.

### ADR-018: Rebrand — "Ministerio de Cooperativas y Mutuales" pasa a ser "Secretaría General de Gobierno"
**Decisión**: El sistema deja de identificarse como "Ministerio de Cooperativas y Mutuales de la Provincia de Córdoba" y pasa a ser un servicio de la **Secretaría General de Gobierno**. Cambia el nombre visible en `LoginPage.tsx`, el header de `Layout.tsx`, y los metadatos de `index.html` (`<title>`, descripción, Open Graph/Twitter). Se actualiza también la documentación viva: root `CLAUDE.md`, `docs/files/CLAUDE.md`, `docs/files/README.md`, `docs/files/spec-infraestructura-gcp.md`, `docs/context/organigrama.md` (nodo raíz del árbol), `services/svc-vivienda/pyproject.toml` y el `info.title`/`description` de `infra/gateway/openapi.yaml`.
**Razón**: Decisión organizacional — el ministerio como entidad dueña del sistema deja de existir con ese nombre; la Secretaría General de Gobierno pasa a ser la titular. Es consistente con que el sistema ya era transversal a varias secretarías (Vivienda, Privada, y las `activa:false` de infraestructura/territorial/desarrollo/gasífera) — "Ministerio de Cooperativas y Mutuales" era, en rigor, el nombre de un área más, no del portal completo.
**Consecuencia**: Los identificadores técnicos (`ministerio-gateway`, `ministerio-api`, `ministerio-config-v*`, `ministerio-postgres`, prefijos `viv_*`/`priv_*`/`portal_*`, el subdominio `ministerio-gateway-3j5k00ma.uc.gateway.dev`, la URL `gestorcooperativo.web.app`) **no se renombran** — son recursos GCP/DNS ya provisionados y renombrarlos es un proyecto de migración aparte, no parte de este ADR. Documentos históricos/archivales (`presentaciones/*.html`, `docs/context/01_plan_de_trabajo.md`, `docs/context/idea_gral.md`, `docs/context/areas/**`, la copia guardada de la web del ministerio en `docs/context/estetica gobierno/`, y los dumps generados `estructura*.txt`) **tampoco se tocan** — son registro de lo que existía/se propuso en su momento bajo el nombre viejo, no documentación viva. Si en el futuro se decide renombrar los recursos GCP, requiere spec propio (migración de dominio/DNS + gateway host + posible downtime).

### ADR-019: Panel de notificaciones internas — módulo transversal en svc-vivienda
**Decisión**: Las alertas internas del sistema (fallos de sincronización, avisos operativos, novedades) se centralizan en un módulo transversal `app/notificaciones/` dentro de `svc-vivienda`, montado en `/api/v1/notificaciones` (sin prefijo `/vivienda`), con su frontend en `src/modules/notificaciones/` y una entrada de navegación top-level + una campana con contador en la barra superior. Modelo de datos: **feed global** (`portal_notificaciones`, una fila por evento) + **estado de lectura por usuario** (`portal_notificacion_lecturas`, sin fila = no leída); no hay bandeja por-destinatario. Cada notificación puede ser `global` / dirigida a un `rol` / dirigida a una `secretaria`; el filtro por visibilidad se aplica en la lectura (`Admin`/`Autoridad` ven todo), igual que `resumen_territorial`. La emisión se hace por `POST /internal/notificaciones` (IAM-only, no expuesto por el Gateway — mismo patrón que ADR-015) para jobs (Cloud Scheduler) u otros servicios; a futuro puede pasar a consumir eventos Pub/Sub. No hay alta desde la UI en v1.
**Razón**: Extiende ADR-007 a una tercera funcionalidad transversal — `svc-vivienda` es el único servicio con Cloud SQL activo, y crear `svc-notificaciones` (el sketch descartado de `docs/context/organigrama.md`) es sobre-ingeniería a esta escala. El feed global evita el fan-out por usuario del emisor; el estado de lectura por usuario es la única parte que sí es por-persona. Mantener la emisión detrás de un endpoint interno IAM desacopla "quién genera la alerta" de "quién la lee" y no requiere tocar el Gateway cada vez que un job nuevo quiera notificar.
**Consecuencia**: Dos tablas nuevas con prefijo `portal_*` (precedente: la funcionalidad transversal que además es adyacente a la identidad de usuario usa su propio prefijo, como `portal_usuarios`). Migración `0028`. El guard de borrado/baja lógica de notificaciones **no** existe en v1 (no hay UI de administración); si se agrega, debe considerar que un `destino_tipo` mal elegido sólo afecta visibilidad, no integridad. Email/push, dropdown de la campana y alta manual desde la UI quedan fuera de v1. Spec: `docs/files/spec-notificaciones.md`. Primer consumidor real, además de Cloud Scheduler: la vinculación Vivienda → Privada (ADR-020), que llama a `notificaciones_service.crear(...)` **en proceso** (no por HTTP — vive en el mismo servicio) para las alertas de "campos corregidos" / "pendiente de revisión".

### ADR-020: Primera escritura cross-servicio (Vivienda → Privada) vía endpoint interno IAM-only idempotente
**Decisión**: Al crear o editar un caso en los paneles DGV de `svc-vivienda` (Cordón Cuneta, Córdoba Hogar, Mi Lugar), el servicio llama de forma síncrona e inline (dentro de `crear_*`/`actualizar_*`, misma transacción) a un endpoint interno IAM-only nuevo de `svc-privada` — `POST /internal/privada/gestiones/sync` — que crea o vincula/sincroniza una gestión correspondiente. Es la primera vez que un servicio **escribe** en el otro por `/internal/**` (hasta ahora sólo había lecturas: `GET /internal/portal/usuarios/{email}` de ADR-015, `GET /internal/privada/rollup-territorial` de ADR-016). Tres restricciones de diseño hacen esto seguro sin transacción distribuida entre bases separadas (ADR-001):
1. **Best-effort desde el llamador**: la llamada nunca bloquea ni hace fallar el alta/edición del caso de origen — cualquier fallo se loguea y se registra en `viv_privada_sync_log` (tabla nueva en `svc-vivienda`), igual criterio tolerante que `resumen_territorial.fetch_privada_lineas` (ADR-016) pero en sentido push.
2. **Idempotencia por construcción**: `svc-vivienda` calcula `id_legacy = f"vivienda:{caso_tipo}:{caso_id}"` de forma determinística (sin ronda previa); `svc-privada` resuelve "¿ya sincronicé este caso?" con un único `SELECT` sobre `priv_gestiones.id_legacy` (`UNIQUE` desde la migración `0001`) — un reintento nunca duplica. El matching cuando no hay vínculo previo sigue el orden `nro_expediente` → `localidad+departamento` (sólo gestiones abiertas, sin `id_legacy` de otro caso); con 2+ candidatos ambiguos no se auto-vincula ni se crea nada (`PENDING_REVIEW`) — mismo riesgo de colisión ya documentado en ADR-016/`spec-resumen-territorial-ficha-localidad.md`.
3. **Escritura acotada (whitelist)**: el sync sólo puede tocar 7 campos "derivados" de la gestión (`categoria_id`/`programa_id`/`area_id`/`ministerio_agencia_id`/`ok_gobernador`/`ok_ministro`/`nro_expediente`, este último sólo si el caso lo tiene) — nunca `detalle`, `observaciones`, `urgencia`, `estado` ni nada que un operador de Privada haya cargado a mano.
**Razón**: Con svc-privada en el mismo proyecto GCP (post-migración, ADR-008), extender el patrón de endpoint interno IAM-only (en vez de Pub/Sub o un job batch) mantiene la propagación simple y sin infraestructura nueva, igual que ADR-016 lo hizo para la lectura. El volumen es bajo (altas manuales de operador, pocas por semana) — no justifica un bus de mensajería, y `svc-privada` deliberadamente no emite/consume Pub/Sub (ADR-009).
**Consecuencia**: El estado del vínculo vive enteramente del lado del llamador — `viv_privada_vinculos` (snapshot, 1 fila por caso, `(caso_tipo, caso_id)` sin FK real, mismo patrón polimórfico que `ChecklistTecnico.programa`/`entidad_id`) + `viv_privada_sync_log` (append-only, 1 fila por intento) en `svc-vivienda`, migración `0029`. `svc-privada` no modela "caso de Vivienda vinculado" — sólo guarda el `id_legacy` recibido. Un `ERROR` transitorio nunca degrada un vínculo que ya estaba `LINKED` (evita que un timeout puntual dispare una alerta de "revisión pendiente" sobre un caso que en realidad está bien vinculado). Las alertas al panel de notificaciones (ADR-019) sólo se emiten en dos casos: se corrigieron campos de una gestión ya vinculada (`destino_tipo=secretaria`, `destino_valor=privada`), o el caso queda `PENDING_REVIEW` — nunca por un `ERROR` transitorio. `ministerio_agencia_id="MIN_GOBIERNO"` confirmado 2026-09-14 contra el catálogo real (`docs/data/cat_ministerio_agencia.json`, volcado de `priv_cat_ministerio_agencia`). `services/cloudbuild.yaml` enciende `PRIVADA_SYNC_GESTIONES_ENABLED=true` por default en cada deploy de CI (mismo criterio que `_PRIVADA_FETCH_ENABLED`, ADR-016); el default en `config.py` sigue en `False` para dev/local. Spec: `docs/files/spec-vinculacion-vivienda-privada.md`.

### ADR-021: resumen_territorial federa una tercera área, Gasífera (mismo patrón que ADR-016)
**Decisión**: `resumen_territorial` agrega `svc-gasifera` como tercera fuente federada, replicando ADR-016 al pie de la letra: `settings.gasifera_fetch_enabled = True` + `svc_gasifera_internal_url` seteada → `fetch_gasifera_lineas()` llama a `GET /internal/gasifera/rollup-territorial` (nuevo, IAM-only, rollup por `(departamento, localidad)` sobre `gas_pit_acciones_territorio` — no incluye `gas_pit_obras`, mismo alcance acotado que el rollup de Privada, que tampoco trae todas sus tablas) con un ID token de la SA de `svc-vivienda`. Requiere el grant inverso al de ADR-015: `svc-vivienda@` con `roles/run.invoker` sobre `svc-gasifera` (ADR-015 ya tenía el sentido opuesto, `svc-gasifera@` invoker sobre `svc-vivienda`, para que Gasífera resuelva `portal_usuarios`).
**Razón**: El usuario pidió explícitamente que las obras/acciones de gas "matcheen" en Resumen Territorial. El matching territorial en todo el sistema (Vivienda y Privada) es por **texto normalizado** `(departamento, localidad)`, nunca por un id de catálogo geográfico (`viv_geo_localidades.id_geo`/`priv_geo_localidades.id_geo` existen pero no se usan como llave de join en ningún lado del pipeline actual) — replicar "el mismo patrón" significa seguir ese criterio, no introducir uno nuevo. `svc-gralgob` había dejado esta misma federación explícitamente fuera de alcance en su propio spec (`spec-sync-atp-compromiso-gobernador.md §12.8` y su sección "Fuera de alcance"), señalando que "requiere un ADR y spec propios" — este ADR es ese paso, para Gasífera.
**Consecuencia**: Mismo modo de falla que Privada (ADR-016): una caída de `svc-gasifera` produce un snapshot parcial (sin la línea de gas) hasta el próximo recálculo, nunca tumba el cómputo de Vivienda/Privada. `aggregations.py` gana `resumen_gasifera_estado`/`detalle_gasifera` (cerradas = `CUMPLIDO`, activas = `PENDIENTE`/`EN EJECUCIÓN`) y suma `"gasifera"` a `_AREA_ORDER`/`_PROGRAMA_ORDER`/`PROGRAMA_LABEL` — no requiere cambios en `filtrar_por_visibilidad` (ya filtra genéricamente por `area in secretarias_del_usuario`, y `"gasifera"` ya estaba en `SECRETARIAS_VALIDAS`). `services/cloudbuild.yaml` necesita `_GASIFERA_FETCH_ENABLED`/`_SVC_GASIFERA_INTERNAL_URL` nuevas (mismo criterio que las de Privada). Spec: `docs/files/spec-sync-gasifera-pit.md §15` (endpoint nuevo del lado Gasífera) + nota en `docs/files/spec-resumen-territorial.md` (tercera área federada).

### ADR-022: resumen_territorial federa una cuarta área, ATP/Gralgob (mismo patrón que ADR-016/ADR-021)
**Decisión**: `resumen_territorial` agrega `svc-gralgob` (módulo `atp`) como cuarta fuente federada, replicando ADR-021 al pie de la letra: `settings.atp_fetch_enabled = True` + `svc_gralgob_internal_url` seteada → `fetch_atp_lineas()` llama a `GET /internal/atp/rollup-territorial` (nuevo, IAM-only, rollup por `(departamento, localidad)` sobre `atp_compromisos` + `atp_cronograma_pagos`) con un ID token de la SA de `svc-vivienda`. Requiere el grant inverso al de ADR-015: `svc-vivienda@` con `roles/run.invoker` sobre `svc-gralgob` (ADR-015 ya tenía el sentido opuesto, `svc-gralgob@` invoker sobre `svc-vivienda`, para que Gralgob resuelva `portal_usuarios`).
**Razón**: El usuario pidió explícitamente que ATP "matchee" en Resumen Territorial, mismo pedido que ya había hecho para Gasífera (ADR-021). `svc-gralgob` había dejado esta federación fuera de alcance en su propio spec (`spec-sync-atp-compromiso-gobernador.md §12.9`, nota de "para más adelante") — este ADR es ese paso. A diferencia de Privada/Gasífera (badge derivado de un conteo por estado), ATP no tiene estados de catálogo: el badge (`resumen_atp_estado`) se deriva de cuánto del monto anunciado ya fue entregado según el cronograma de pagos (`Pendiente`/`Parcial`/`Pagado`), mismo espíritu (activo/mixto/cerrado) con una métrica distinta porque el dominio es distinto.
**Consecuencia**: Mismo modo de falla que Privada/Gasífera: una caída de `svc-gralgob` produce un snapshot parcial (sin la línea de ATP) hasta el próximo recálculo, nunca tumba el cómputo de las otras áreas. `aggregations.py` gana `resumen_atp_estado`/`detalle_atp` y suma `"gralgob"`/`"atp"` a `_AREA_ORDER`/`_PROGRAMA_ORDER`/`PROGRAMA_LABEL` — no requiere cambios en `filtrar_por_visibilidad` (ya filtra genéricamente por `area in secretarias_del_usuario`, y `"gralgob"` ya estaba en `SECRETARIAS_VALIDAS`). `services/cloudbuild.yaml` necesita `_ATP_FETCH_ENABLED`/`_SVC_GRALGOB_INTERNAL_URL` nuevas (mismo criterio que Privada/Gasífera). Spec: `docs/files/spec-sync-atp-compromiso-gobernador.md §13` (endpoint nuevo del lado Gralgob) + nota en `docs/files/spec-resumen-territorial.md` (cuarta área federada).

### ADR-023: Notificación cross-servicio en sentido inverso — gralgob/gasifera avisan a svc-vivienda cuando el sync detecta una fila nueva
**Decisión**: `svc-gralgob` (módulo `atp`) y `svc-gasifera` (módulo `gas_pit`) llaman, al final de `sync_from_sheet` (después de loguear la corrida, fuera del `SAVEPOINT` por fila), a `POST /internal/notificaciones` de `svc-vivienda` por cada fila que el `is_new` que ya calculaba cada `_upsert_*` marcó como nueva — nunca por una fila actualizada. Módulo nuevo `app/integrations/notificaciones_vivienda.py` en cada servicio, mismo patrón de ID token que `app/auth.py::_fetch_portal_user` (ADR-015) pero en sentido inverso: acá gralgob/gasifera son quienes llaman, no quienes son llamados. Reusa `settings.svc_vivienda_internal_url` (ya existe en ambos, sembrada para el lookup de `portal_usuarios`) — no hace falta una URL nueva, sólo un flag de activación propio, `notificar_fila_nueva_enabled` (mismo criterio que `privada_sync_gestiones_enabled`, ADR-020), para poder prender/apagar esto sin tocar la config de auth. Mensajes con `destino_tipo="secretaria"`, `destino_valor="gralgob"`/`"gasifera"` respectivamente, siempre citando localidad y departamento de la fila.
**Razón**: Es la primera vez que una notificación se origina en un servicio que no es `svc-vivienda` — hasta ahora `POST /internal/notificaciones` sólo lo llamaba Cloud Scheduler y, en proceso, la propia vinculación Privada (ADR-020). El punto de "fila nueva" ya estaba resuelto en el sync (`is_new`, ver ADR-021/ADR-022 y sus specs) — no hace falta inferirlo de nuevo ni agregar un flag al Sheet. Se dispara después del loop (no dentro del `SAVEPOINT` por fila) para no atarle latencia de red a cada fila del batch ni arriesgar que un fallo de notificación aborte el upsert de una fila válida.
**Consecuencia**: Ninguna migración nueva — todos los campos citados (`departamento`/`localidad`/`fecha_anuncio` en ATP; `departamento`/`localidad`/`accion` en gas_pit) ya existen como columnas. Gasífera sólo notifica desde la hoja "ACCIONES TERRITORIO" (`gas_pit_acciones_territorio`), nunca desde "MATRIZ" (`gas_pit_obras`) — el pedido original era específicamente sobre "obra de gas {acción}", que mapea al campo `accion` de esa tabla. Requiere el mismo grant IAM que ya usa ADR-015 (`svc-gralgob@`/`svc-gasifera@` con `roles/run.invoker` sobre `svc-vivienda`) — no es un permiso nuevo, es el mismo reusado para un endpoint distinto del mismo servicio destino. `services/cloudbuild.yaml` gana `_NOTIFICAR_FILA_NUEVA_ENABLED` (una sola substitución, misma env var en ambos servicios). De paso, los tres mensajes de notificación de la vinculación Privada (ADR-020) se reformularon para citar localidad y departamento explícitamente, por consistencia con estos dos nuevos. Specs: `docs/files/spec-sync-atp-compromiso-gobernador.md` y `docs/files/spec-sync-gasifera-pit.md` (sección nueva en cada uno).
